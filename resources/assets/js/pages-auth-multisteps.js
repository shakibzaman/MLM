        ...
        fields: {
          multiStepsUsername: {
            validators: {
              ...
              stringLength: {
                min: 8,
                max: 30,
                message: 'The name must be more than 8 and less than 30 characters long'
              },
              regexp: {
                regexp: /^[a-zA-Z0-9]+$/,
                message: 'The name can only consist of alphabetical and numeric characters (no spaces allowed)'
            },
              remote: {
                url: '/check-username', // API route to check the email
                type: 'GET', // Ensure POST method is used
                data: function () {
                  const usernameValue = stepsValidationFormStep1.querySelector('[name="multiStepsUsername"]').value;
                  console.log("Checking username:", usernameValue); // Log the email being checked
                  return { username: usernameValue };
                },
                headers: {
                  'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                },
                delay: 500, // Delay the request to avoid flooding the server
          ...
          multiStepsEmail: {
            validators: {
              ...
              remote: {
                url: '/check-email', // API route to check the email
                type: 'GET', // Ensure POST method is used
                data: function () {
                  const emailValue = stepsValidationFormStep1.querySelector('[name="multiStepsEmail"]').value;
                  console.log("Checking email:", emailValue); // Log the email being checked
          ...
          multiStepsPass: {
            validators: {
              ...
              stringLength: {
                min: window.passwordConstraints?.min || 8 ,
                max: window.passwordConstraints?.max || 15,
                message: `The password must be between ${window.passwordConstraints?.min || 8} and ${window.passwordConstraints?.max || 15} characters long`
              }
            }
          },
