...
class CustomerController extends Controller
{
  ...
  {
    $customers = Customer::select(['id', 'first_name','last_name', 'email', 'balance', 'status', 'ip_address', 'lifetime_package', 'monthly_package_status']);
    if ($request->name) {
    ...
    return DataTables::of($customers)
      ...
      ->addColumn('lifetime_package', function ($customer) {
        if ($customer->lifetime_package == null) {
          return '<label class="switch annual switch-danger">
                            <input type="checkbox" class="switch-input" checked="" data-customer-id="' . $customer->id . '">
          ...
          return '<label class="switch annual switch-success">
                            <input type="checkbox" class="switch-input" checked="" data-customer-id="' . $customer->id . '">
                            <span class="switch-toggle-slider">
                              <span class="switch-on">
                                <i class="ti ti-check"></i>
      ...
      ->addColumn('monthly_package_status', function ($customer) {
        return $customer->monthly_package_status == 'active'
          ? '<label class="switch monthly switch-success">
                            <input type="checkbox" class="switch-input" checked="" data-customer-id="' . $customer->id . '">
                            <span class="switch-toggle-slider">
                              <span class="switch-on">
                                <i class="ti ti-check"></i>
          ...
          : '<label class="switch monthly switch-danger">
                            <input type="checkbox" class="switch-input" checked="" data-customer-id="' . $customer->id . '">
                            <span class="switch-toggle-slider">
                              <span class="switch-on">
                                <i class="ti ti-check"></i>
  ...
  {
    $customer = Customer::with(['subscribers' => function($query) {
  ...
  public function update($id, Request $request)
  {
    $this->validate($request, [
      ...
      'email' => 'required|email|unique:customers,email,' . $id, // Use the customer's ID here
      'password' => 'nullable|min:8', // Assuming password is optional for update
      'country_id' => 'required|exists:countries,id',
      'city' => 'required|max:255',
      'phone' => 'required|unique:customers,phone,' . $id, // Use the customer's ID here
    ...
    $data['email_verified_at'] = $request->has('email_verified_at') ? date('Y-m-d') : null;
    $data['document_verified'] = $request->has('document_verified') ? 1 : 0;
    $data['auth_2fa'] = $request->has('auth_2fa') ? 1 : 0;
  ...
  {
    $customers = Customer::select(['id', 'name','first_name','last_name', 'email', 'balance', 'status', 'ip_address', 'lifetime_package', 'monthly_package_status'])
  ...
  public function store(Request $request)
  {
    ...
    $this->validate($request, [
      'first_name' => 'required|max:255',
      'last_name' => 'required|max:255',
      'name' => 'required|max:255|unique:customers|regex:/^\S*$/',
    ...
    $data['reference_user'] = 1;
    $data['init_lifetime_package'] = 1;
  ...
  public function topRecruiterData()
  {
    ...
    ])->having('active_subscribers_count', '>', 0)->orderBy('active_subscribers_count', 'desc');
  ...
  {
    return view('admin.customer.ratingScore');
  }

  public function ratingScoreData()
  {
    $customers = Customer::select('*');

    return DataTables::of($customers)
      ->addColumn('reward_point', function ($customer) {
        $rating = new RatingService();
        return $rating->calculateRating($customer);
      })
      ->make(true);
  }
